...
# Spec Master (Kimi Code entrypoint)
...
for Kimi Code (`.kimi-code/skills/<name>/SKILL.md`), invoked as
`/skill:spec-master <context-file>`.
...
1. **Argument**: the context-file path is whatever follows `/skill:spec-master`
   in the invocation (e.g. `/skill:spec-master CLAUDE.md`). If missing
   or the file doesn't exist, stop and explain the usage: `/skill:spec-master <context-file>`.
...
4. **Running an actual Spec Kit phase**: this repository's Spec Kit
   ...
   `.kimi-code/skills/speckit-<phase>/SKILL.md`. Read that skill and follow it with
...
Re-invoking `/skill:spec-master <context-file>` follows the exact same Step 0 (state show + fingerprint
