...
<!-- Generated entrypoint for Kiro CLI — see spec-master/adapters/generic.md -->
...
> **Note:** Kiro CLI does not substitute $ARGUMENTS in file-based prompts (see github/spec-kit#1926) — type the context-file path in the chat message itself, e.g. '/spec-master' then 'CLAUDE.md' on the next line, rather than relying on the placeholder.
...
4. **Executando uma fase real do Spec Kit**: leia o comando/skill
   `speckit-<fase>` que o Spec Kit instalou em `.kiro/prompts/` para este
